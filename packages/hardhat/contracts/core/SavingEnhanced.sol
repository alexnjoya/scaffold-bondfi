...
contract AjoEsusuSavings is ReentrancyGuard, Ownable, Pausable {
    ...
    // Reference to the DEX/Swap contract
    Swap public swapContract;
    
    // Base currency for each group (the unified currency all deposits convert to)
    mapping(uint256 => address) public groupBaseCurrency;
    
    // User's preferred payout currency for each group
    mapping(uint256 => mapping(address => address)) public userPayoutPreference;
    
    // Track original deposit currencies for analytics
    mapping(uint256 => mapping(address => mapping(uint256 => address))) public depositCurrency; // groupId => user => round => currency
    
    // Emergency fallback if swap fails
    mapping(uint256 => mapping(address => uint256)) public emergencyBalances; // groupId => token => amount
    ...
    event PayoutCurrencySet(
        uint256 indexed groupId,
        address indexed member,
        address preferredCurrency
    );
    
    event SwapContractUpdated(address oldContract, address newContract);
    ...
    constructor(address[] memory _supportedTokens, string[] memory _tokenNames, address _swapContract) Ownable(msg.sender) {
